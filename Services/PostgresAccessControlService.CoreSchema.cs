...
public sealed partial class PostgresAccessControlService : IAccessControlService
{
    private async Task EnsureSchemaAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var sql = BuildEnsureSchemaSql(usersTable);
    ...
    private static string BuildEnsureSchemaSql(string usersTable)
    {
        var sb = new StringBuilder();

        sb.AppendLine($"ALTER TABLE {usersTable} ADD COLUMN IF NOT EXISTS full_name VARCHAR(160);");
        sb.AppendLine($"ALTER TABLE {usersTable} ADD COLUMN IF NOT EXISTS email VARCHAR(255);");
        sb.AppendLine($"ALTER TABLE {usersTable} ADD COLUMN IF NOT EXISTS default_company_id BIGINT;");
        sb.AppendLine($"ALTER TABLE {usersTable} ADD COLUMN IF NOT EXISTS default_location_id BIGINT;");
...
CREATE OR REPLACE FUNCTION fn_validate_user_default_scope()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
ALTER TABLE sec_modules ADD COLUMN IF NOT EXISTS module_code VARCHAR(80);
ALTER TABLE sec_modules ADD COLUMN IF NOT EXISTS module_name VARCHAR(160);
ALTER TABLE sec_modules ADD COLUMN IF NOT EXISTS sort_order INT NOT NULL DEFAULT 100;
ALTER TABLE sec_modules ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT TRUE;
ALTER TABLE sec_modules ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT NOW();
ALTER TABLE sec_modules ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW();

ALTER TABLE sec_submodules ADD COLUMN IF NOT EXISTS submodule_code VARCHAR(80);
ALTER TABLE sec_submodules ADD COLUMN IF NOT EXISTS submodule_name VARCHAR(160);
ALTER TABLE sec_submodules ADD COLUMN IF NOT EXISTS sort_order INT NOT NULL DEFAULT 100;
ALTER TABLE sec_submodules ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT TRUE;
ALTER TABLE sec_submodules ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT NOW();
ALTER TABLE sec_submodules ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW();

ALTER TABLE sec_actions ADD COLUMN IF NOT EXISTS action_code VARCHAR(80);
ALTER TABLE sec_actions ADD COLUMN IF NOT EXISTS action_name VARCHAR(160);
ALTER TABLE sec_actions ADD COLUMN IF NOT EXISTS sort_order INT NOT NULL DEFAULT 100;
ALTER TABLE sec_actions ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT TRUE;
ALTER TABLE sec_actions ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT NOW();
ALTER TABLE sec_actions ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW();
...
BEGIN
    IF EXISTS (
        ...
        ALTER TABLE sec_modules RENAME COLUMN code TO module_code;
    END IF;
    ...
    IF EXISTS (
        ...
        ALTER TABLE sec_modules RENAME COLUMN name TO module_name;
    END IF;
    ...
    IF EXISTS (
        ...
        EXECUTE $fix$
            UPDATE sec_modules
            SET module_code = COALESCE(NULLIF(btrim(module_code), ''), code)
            WHERE code IS NOT NULL;
        $fix$;
        ALTER TABLE sec_modules DROP COLUMN code CASCADE;
    END IF;
    ...
    IF EXISTS (
        ...
        EXECUTE $fix$
            UPDATE sec_modules
            SET module_name = COALESCE(NULLIF(btrim(module_name), ''), name)
            WHERE name IS NOT NULL;
        $fix$;
        ALTER TABLE sec_modules DROP COLUMN name CASCADE;
    END IF;

    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = current_schema()
          AND table_name = 'sec_submodules'
          AND column_name = 'code'
    ) AND NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = current_schema()
          AND table_name = 'sec_submodules'
          AND column_name = 'submodule_code'
    ) THEN
        ALTER TABLE sec_submodules RENAME COLUMN code TO submodule_code;
    END IF;
    ...
    IF EXISTS (
        ...
        WHERE table_schema = current_schema()
          AND table_name = 'sec_submodules'
          AND column_name = 'submodule_name'
    ) THEN
        ALTER TABLE sec_submodules RENAME COLUMN name TO submodule_name;
    END IF;
    ...
    IF EXISTS (
        ...
        EXECUTE $fix$
            UPDATE sec_submodules
            SET submodule_code = COALESCE(NULLIF(btrim(submodule_code), ''), code)
            WHERE code IS NOT NULL;
        $fix$;
        ALTER TABLE sec_submodules DROP COLUMN code CASCADE;
    END IF;
    ...
    IF EXISTS (
        ...
        EXECUTE $fix$
            UPDATE sec_submodules
            SET submodule_name = COALESCE(NULLIF(btrim(submodule_name), ''), name)
            WHERE name IS NOT NULL;
        $fix$;
        ALTER TABLE sec_submodules DROP COLUMN name CASCADE;
    END IF;

    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = current_schema()
          AND table_name = 'sec_actions'
          AND column_name = 'code'
    ) AND NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = current_schema()
        ...
        ALTER TABLE sec_actions RENAME COLUMN code TO action_code;
    END IF;
    ...
    IF EXISTS (
        ...
        ALTER TABLE sec_actions RENAME COLUMN name TO action_name;
    END IF;
    ...
    IF EXISTS (
        ...
        EXECUTE $fix$
            UPDATE sec_actions
            SET action_code = COALESCE(NULLIF(btrim(action_code), ''), code)
            WHERE code IS NOT NULL;
        $fix$;
        ALTER TABLE sec_actions DROP COLUMN code CASCADE;
    END IF;
    ...
    IF EXISTS (
        ...
        EXECUTE $fix$
            UPDATE sec_actions
            SET action_name = COALESCE(NULLIF(btrim(action_name), ''), name)
            WHERE name IS NOT NULL;
        $fix$;
        ALTER TABLE sec_actions DROP COLUMN name CASCADE;
    END IF;
END
$$;
...
WITH submodule_seeds(module_code, submodule_code, submodule_name, sort_order, is_active) AS (
    VALUES
        ...
        ('inventory', 'reports', 'Reports', 100, TRUE),
        ('inventory', 'api_inv', 'API Inv', 110, TRUE),
...
SELECT set_config('app.role_policy_bypass', 'on', TRUE);
...
SELECT set_config('app.role_policy_bypass', 'off', TRUE);
");
...
DROP TRIGGER IF EXISTS trg_validate_user_default_scope ON {usersTable};
CREATE TRIGGER trg_validate_user_default_scope
BEFORE INSERT OR UPDATE ON {usersTable}
FOR EACH ROW
EXECUTE FUNCTION fn_validate_user_default_scope();
");
